...
public class SyncExtensionsTests
{
    ...
    public void QuerySyncTerminators_Work()
    {
        ...
        db.Things.lstS().Count.eq(2);
        db.Things.w(t => t.Rank == 1).oneS()!.Name.eq("a");
        db.Things.w(t => t.Rank == 2).singleS()!.Name.eq("b");
        db.Things.anyS().tru();
        db.Things.cntS().eq(2);
    }
    ...
    public void AddS_PersistsAndReturns()
    {
        ...
        (saved.Id > 0).tru();
        db.Things.idS(saved.Id)!.Name.eq("x");
    }
    ...
    public void UpdS_And_DelS_Work()
    {
        ...
        saved.Name = "y";
        db.updS(saved);
        db.Things.idS(saved.Id)!.Name.eq("y");
        db.delS(saved);
        db.Things.cntS().eq(0);
    }
